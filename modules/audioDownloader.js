...
async function solveRecaptcha(page) {
  ...
  // スクショ
  {
    ...
    console.log(`[reCAPTCHA] 🖼️ challenge iframe スクショ: tmp/${path.basename(debugShot1)}`);
  }
  ...
  for (const sel of [
    ...
    try {
      ...
      console.log(`[reCAPTCHA] ⚠️ '${sel}' 取得失敗→次`);
    }
  }
  if (!toggled) {
  ...
  for (const sel of [
    ...
    try {
      ...
      console.log(`[reCAPTCHA] ⚠️ '${sel}' 未取得→次`);
    }
  }
  if (!played) {
